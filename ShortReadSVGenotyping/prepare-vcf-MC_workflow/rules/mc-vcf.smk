...
for callset in config['nr_samples'].keys():
	...
	min_an = 1 #int(0.8 * total * 2)
	min_an_male = 1 #int(0.8 * ( ((total-males)*2) + males) )
	min_an_female = 1 #int(0.8 * males * 2)
	
	nr_samples[callset] = [min_an, min_an_male, min_an_female]

reference_to_ignore = "NDH108" #"CHM13" if config['reference_version'] == "GRCh38" else "GRCh38"


chromosomes = ['A01','A02','A03','A04','A05','A06','A07','A08','A09','A10','B01','B02','B03','B04','B05','B06','B07','B08','B09','B10']# [config['reference_prefix'] + str(i) for i in range(1,21)] # + ['chrX']
...
rule filter_vcf:
	...
	shell:
		"bcftools view --samples ^{reference_to_ignore} --force-samples  {input} | bcftools view --min-ac 1 | python3 scripts/filter-vcf.py {min_an} {min_an_male} {min_an_female} --chromosomes {chromosomes} 2> {log} 1> {output}"
